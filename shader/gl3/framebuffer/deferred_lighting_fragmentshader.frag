...
//
vec4 computeSpecularLight(in int i, in vec3 normal, in vec3 position, in Material material) {
...
//
vec4 computeSpecularLights(in vec3 normal, in vec3 position, in Material material) {
	...
	for (int i = 0; i < MAX_LIGHTS; i++) {
		...
		// compute light
		fragColor+= computeSpecularLight(i, normal, position, material);
	}
	//
	return fragColor;
}
...
void main(void) {
	...
	if (type == 0) {
		...
		vec4 fragColor = material.emission + computeSpecularLights(normal, position, material);
