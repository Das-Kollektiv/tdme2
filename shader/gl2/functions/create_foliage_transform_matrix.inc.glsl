mat4 createFoliageTransformMatrix(vec3 origin, vec3 vertex, vec3 translation) {
	...
	float zAxisRotation = amplitudeDefault + sin((clamp((((zAxisRotation < 160.0 / 2.0?zAxisRotation:160.0 - zAxisRotation) - (160.0 / 4.0)) * 1.0 / 8.0) * ((vertex.y - origin.y) * 2.0), -20.0, +20.0)) / 20.0 * 3.14) * amplitudeMax;
	return
