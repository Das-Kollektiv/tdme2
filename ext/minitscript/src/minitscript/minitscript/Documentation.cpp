...
const set<string> Documentation::getAllClassMethods(MinitScript* minitScript) {
	set<string> allClassMethods;
	//
	for (auto typeIdx = static_cast<int>(MinitScript::TYPE_STRING); ; typeIdx++) {
	...
	for (auto scriptMethod: minitScript->getMethods()) {
		...
		auto _class = false;
		for (auto typeIdx = static_cast<int>(MinitScript::TYPE_STRING); ; typeIdx++) {
			const auto& classNameCandidate = MinitScript::Variable::getTypeAsString(static_cast<MinitScript::VariableType>(typeIdx));
		...
		auto _static =
			...
			MinitScript::Variable::getTypeAsString(scriptMethod->getArgumentTypes()[0].type) != className;
		//
...
const set<string> Documentation::getMethodsCategories(MinitScript* minitScript, const set<string>& allClassMethods, MinitScript* excludeMinitScript) {
	set<string> categories;
	auto scriptMethods = minitScript->getMethods();
	for (auto scriptMethod: scriptMethods) {
		if (excludeMinitScript != nullptr && excludeMinitScript->hasMethod(scriptMethod->getMethodName()) == true) continue;
...
const string Documentation::generateClassesDocumentation(const string& heading, int mainHeadingIdx, MinitScript* minitScript, Properties& descriptions, const string& descriptionPrefix, const set<string>& allClassMethods) {
	auto scriptMethods = minitScript->getMethods();
	//
	...
	for (auto typeIdx = static_cast<int>(MinitScript::TYPE_STRING); ; typeIdx++) {
		const auto& className = MinitScript::Variable::getTypeAsString(static_cast<MinitScript::VariableType>(typeIdx));
		if (className.empty() == true) break;
		auto classNameDescription = descriptions.get("minitscript.baseclass." + (className.empty() == true?"No class":className), "Not documented");
		//
		result+= "\n";
...
const string Documentation::generateOperatorsDocumentation(
	...
	// operators
	auto scriptOperatorMethods = minitScript->getOperatorMethods();
	vector<string> operators;
	for (auto scriptMethod: scriptOperatorMethods) {
		string operatorString;
		operatorString = "| ";
		operatorString+= StringTools::replace(MinitScript::getOperatorAsString(scriptMethod->getOperator()), "|", "\\|");
		...
		operatorString+= "): ";
		operatorString+= MinitScript::Variable::getReturnTypeAsString(scriptMethod->getReturnValueType(), scriptMethod->isReturnValueNullable());
...
const string Documentation::generateVariablesDocumentation(
	...
	for (const auto& [variableName, variable]: minitScript->getRootScriptState().variables) {
