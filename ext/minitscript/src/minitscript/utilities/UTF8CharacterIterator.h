 ...
 */
class minitscript::utilities::UTF8CharacterIterator {
public:
	/**
	 * UTF8 cache entry
	 */
	...
	inline void seekBinaryPosition(int64_t position) const {
		...
		if (position >= UTF8PositionCache::CACHE_ENTRY_SIZE && cache != nullptr && cache->binaryCache.empty() == false) {
			auto cacheIdx = ::minitscript::math::Math::min((position / UTF8PositionCache::CACHE_ENTRY_SIZE) - 1, cache->binaryCache.size() - 1);
	...
	inline void seekCharacterPosition(int64_t position) const {
		...
		if (position >= UTF8PositionCache::CACHE_ENTRY_SIZE && cache != nullptr && cache->characterCache.empty() == false) {
			auto cacheIdx = ::minitscript::math::Math::min((position / UTF8PositionCache::CACHE_ENTRY_SIZE) - 1, cache->characterCache.size() - 1);
