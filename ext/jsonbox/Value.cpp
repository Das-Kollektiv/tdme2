/**
 * @version $Id: a69850571b0dd0090a52aa3ab99352dfca5c30a3 $
 */
...
const tdme::ext::jsonbox::Array Value::EMPTY_ARRAY = tdme::ext::jsonbox::Array();
...
Value::Value(const tdme::ext::jsonbox::Array &newArray) : type(ARRAY),
	data(new tdme::ext::jsonbox::Array(newArray)) {
}
...
Value::Value(const Value &src) : type(src.type), data() {
	...
	case ARRAY:
		data.arrayValue = new tdme::ext::jsonbox::Array(*src.data.arrayValue);
		break;
...
void Value::setArray(const tdme::ext::jsonbox::Array &newArray) {
	if (type == ARRAY) {
		*data.arrayValue = newArray;
		...
		clear();
		type = ARRAY;
		data.arrayValue = new tdme::ext::jsonbox::Array(newArray);
	}
}
		...
		while (noErrors && input.good()) {
			...
			if (input.good()) {
				if (currentCharacter == Structural::BEGIN_END_STRING) {
					...
					setArray(tdme::ext::jsonbox::Array());
...
Value::ValueDataPointer::ValueDataPointer(tdme::ext::jsonbox::Array *newArrayValue) :
...
void Value::readArray(std::istream &input, tdme::ext::jsonbox::Array &result) {
	bool notDone = true;
	char currentChar;
...
std::ostream& tdme::ext::jsonbox::operator<<(std::ostream &output, const Value &v) {
	switch (v.type) {
	case Value::STRING:
