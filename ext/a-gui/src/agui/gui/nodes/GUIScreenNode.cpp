...
#include <agui/gui/events/GUIDragRequestListener.h>
#include <agui/gui/events/GUIFocusListener.h>
#include <agui/gui/events/GUIInputEventHandler.h>
#include <agui/gui/events/GUIMouseOverListener.h>
...
void GUIScreenNode::initializeMinitScript() {
	...
	if (script != nullptr && script->hasFunction("initialize") == true) {
		vector<MinitScript::Variable> arguments { scriptArguments };
...
void GUIScreenNode::tick() {
	...
	if (scriptOnTickAvailable == true) {
		vector<MinitScript::Variable> arguments(0);
		span argumentsSpan(arguments);
		MinitScript::Variable returnValue;
...
GUITexture* GUIScreenNode::getImage(const string& fileName)
{
	// get canonical file name
	...
	if (image == nullptr) {
		try {
			...
			if (image != nullptr) {
				...
				image->setClampMode(GUITexture::CLAMPMODE_TRANSPARENTPIXEL);
			}
		} catch (Exception& exception) {
