...
GUITexture* PNGTextureReader::read(const string& textureId, const vector<uint8_t>& pngData, bool powerOfTwo, const string& idPrefix) {
	int width;
	int height;
		...
		if (textureWidth != width || textureHeight != height) {
			...
			auto texture =
				...
				>(
					new GUITexture(
						idPrefix + textureId,
						GUITexture::getRGBDepthByPixelBitsPerPixel(bytesPerPixel * 8),
						GUITexture::getPNGFormatByPixelBitsPerPixel(bytesPerPixel * 8),
						width,
						height,
						textureWidth,
						textureHeight,
						GUITexture::getRGBFormatByPixelBitsPerPixel(bytesPerPixel * 8),
