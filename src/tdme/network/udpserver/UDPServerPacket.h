...
public:
	 ...
	 */
	inline const UDPServerPacket* setPosition(uint16_t position) const {
	 ...
	 * @return UDP client packet
	 */
	inline UDPServerPacket* setPosition(uint16_t position) {
		this->position = position;
		return this;
	}
	 ...
	 * @return UDP client packet
	 */
	inline UDPServerPacket* putBool(bool value) {
	 ...
	 * @return UDP client packet
	 */
	inline UDPServerPacket* putInt16(uint16_t value) {
		putByte((value) & 0xFF);
	 ...
	 * @return UDP client packet
	 */
	inline UDPServerPacket* putInt(uint32_t value) {
		putByte((value) & 0xFF);
		putByte((value >> 8) & 0xFF);
	 ...
	 * @return UDP client packet
	 */
	inline UDPServerPacket* putInt64(uint64_t value) {
		putByte((value) & 0xFF);
		putByte((value >> 8) & 0xFF);
		putByte((value >> 16) & 0xFF);
		putByte((value >> 24) & 0xFF);
	 ...
	 * @return UDP client packet
	 */
	inline UDPServerPacket* putFloat(float value) {
	 ...
	 * @return UDP client packet
	 */
	inline UDPServerPacket* putString(const string& value) {
		if (value.size() > 255) {
			Console::println("UDPServerPacket::putString(): string size out of range: string will be clamped to max length of 255 bytes");
		}
	 ...
	 * @return UDP client packet
	 */
	inline const UDPServerPacket* getBytes(uint8_t* bytes, uint16_t byteCount) const {
	 ...
	 * @return UDP client packet
	 */
	inline UDPServerPacket* getBytes(uint8_t* bytes, uint16_t byteCount) {
		for (auto i = 0; i < byteCount; i++) bytes[i] = getByte();
		return this;
	}
	 ...
	 * @return UDP client packet
	 */
	inline UDPServerPacket* putBytes(const uint8_t* bytes, uint16_t byteCount) {
	 ...
	 * @return UDP client packet
	 */
	inline UDPServerPacket* putPacket(const UDPServerPacket* packet) {
