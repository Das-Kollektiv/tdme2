...
bool Float::is(const string& str) {
	...
	return
		str.empty() == false &&
		find_if(trimmedStr.begin() + (trimmedStr[0] == '-'?1:0), trimmedStr.end(), [&dotCount](unsigned char c) { return isdigit(c) == 0 && (c != '.' || ++dotCount > 1); }) == trimmedStr.end();
}
...
float Float::parse(const string& str) {
	...
	return
		(str.empty() == false &&
		find_if(trimmedStr.begin() + (trimmedStr[0] == '-'?1:0), trimmedStr.end(), [&dotCount, &digitSum](unsigned char c) { if (isdigit(c) != 0) digitSum+= c - '0'; return isdigit(c) == 0 && (c != '.' || ++dotCount > 1); }) == trimmedStr.end()) == true && digitSum > 0?stof(trimmedStr):0.0f;
}
