...
public:
	...
	inline void execute() override {
		...
		if (getScriptState().state == STATEMACHINESTATE_NEXT_STATEMENT) {
			...
			if (getScriptState().scriptIdx == 1) printABC(scriptState.statementIdx); else
...
protected:
	...
	/**
	 * Miniscript transpilation of: function: printABC
	 * @param miniScriptGotoStatementIdx MiniScript goto statement index
	 */
	void printABC(int miniScriptGotoStatementIdx);
	...
	/**
	 * Miniscript transpilation for a statement array/map/set initializer for method 'on_nothing', statement index 4, argument indices 1
	 * @param statement statement
	 * @return initialized variable
	 */
	ScriptVariable on_nothing_initializer_s_4_1(const ScriptStatement& statement);
