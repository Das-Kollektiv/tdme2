...
{
	{
		auto velocity = world->getBody("tire")->getLinearVelocity();
		...
		if (keyDown) velocity.setZ(-8.0f); else
			velocity.setZ(0.0f);
		world->getBody("tire")->setLinearVelocity(velocity);
	}
...
void PhysicsTest5::initialize()
{
	...
	try {
		...
		entity = new Object("cone", _cone);
		entity->setContributesShadows(true);
		entity->setReceivesShadows(true);
		entity->setTranslation(Vector3(0.0f, 2.5f, 0.0f));
		entity->setScale(Vector3(7.5f, 7.5f, 7.5f));
		entity->update();
		engine->addEntity(entity);
...
void PhysicsTest5::onKeyDown(int key, int keyCode, int x, int y, bool repeat) {
