...
public:
	...
	inline void execute() override {
		...
		if (getScriptState().state == STATEMACHINESTATE_NEXT_STATEMENT) {
			...
			if (getScriptState().scriptIdx == 1) xxx(scriptState.statementIdx); else
			if (getScriptState().scriptIdx == 2) on_nothing(scriptState.statementIdx); else
			if (getScriptState().scriptIdx == 3) on_error(scriptState.statementIdx); else
				;
		}
		if (getScriptState().running == false) return;
		executeStateMachine();
...
protected:
	...
	/**
	 * Miniscript transpilation of: function: xxx
	 * @param miniScriptGotoStatementIdx MiniScript goto statement index
	 */
	void xxx(int miniScriptGotoStatementIdx);
	...
	/**
	 * Miniscript transpilation for a statement array/map/set initializer for method 'on_nothing', statement index 0, argument index 1
	 * @param statement statement
	 * @return array index
	 */
	ScriptVariable on_nothing_initializer_s_0_1_1(const ScriptStatement& statement);

	/**
	 * Miniscript transpilation for a statement array/map/set initializer for method 'on_nothing', statement index 1, argument index 1
	 * @param statement statement
	 * @return array index
	 */
	ScriptVariable on_nothing_initializer_s_1_1_1(const ScriptStatement& statement);

	/**
	 * Miniscript transpilation for a statement array/map/set initializer for method 'on_nothing', statement index 1, argument index 2
	 * @param statement statement
	 * @return array index
	 */
	ScriptVariable on_nothing_initializer_s_1_2_1(const ScriptStatement& statement);

	/**
	 * Miniscript transpilation for a statement array/map/set initializer for method 'on_nothing', statement index 2, argument index 1
	 * @param statement statement
	 * @return array index
	 */
	ScriptVariable on_nothing_initializer_s_2_1_2(const ScriptStatement& statement);
