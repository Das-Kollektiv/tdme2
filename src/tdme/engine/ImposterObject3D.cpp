...
ImposterObject3D::ImposterObject3D(
	const string& id,
	...
	billboardModels(billboardModels)
{
	...
	for (auto i = 0; i < billboardModels.size(); i++) {
		billboardObjects[i] = new Object3D(id + ".bb-" + to_string(i), billboardModels[i]);
...
void ImposterObject3D::setEngine(Engine* engine)
{
...
void ImposterObject3D::setRenderer(Renderer* renderer)
{
}

void ImposterObject3D::fromTransformations(const Transformations& transformations)
{
...
void ImposterObject3D::setEnabled(bool enabled)
{
...
bool ImposterObject3D::isFrustumCulling() {
	return frustumCulling;
}

void ImposterObject3D::setFrustumCulling(bool frustumCulling) {
...
void ImposterObject3D::dispose()
{
	// delegate to billboard objects
	for (auto billboardObject: billboardObjects) billboardObject->dispose();
}

void ImposterObject3D::initialize()
{
	// delegate to billboard objects
	for (auto billboardObject: billboardObjects) billboardObject->initialize();
}

void ImposterObject3D::setEnableEarlyZRejection(bool enableEarlyZRejection) {
	// delegate to billboard objects
