...
bool TextureReader::readPNGHeader(const vector<uint8_t>& data, int& width, int& height, uint8_t& bytesPerPixel) {
	// create PNG input stream
...
bool TextureReader::readPNG(const vector<uint8_t>& data, ByteBuffer& textureByteBuffer) {
...
Texture* TextureReader::readPNG(const string& textureId, const vector<uint8_t>& data, bool powerOfTwo, const string& idPrefix) {
	...
	if (readPNGHeader(data, width, height, bytesPerPixel) == false) {
		Console::println("TextureReader::readPNG(): " + idPrefix + textureId + ": Could not read PNG header");
		return nullptr;
	}
	...
	if (powerOfTwo == true) {
		...
		if (textureWidth != width || textureHeight != height) {
			...
			if (readPNG(data, textureByteBuffer) == false) {
				Console::println("TextureReader::readPNG(): " + idPrefix + textureId + ": Could not read PNG bitmap data");
				return nullptr;
			}
			...
			for (auto y = 0; y < height; y++) {
				for (auto i = 0; i < (int)textureYIncrement + (int)textureYPixelRest; i++) {
					scaleTextureLine(textureByteBuffer, textureByteBufferScaled, width, textureWidth, bytesPerPixel, y);
					textureY++;
				}
			...
			while (textureY < textureHeight) {
				scaleTextureLine(textureByteBuffer, textureByteBufferScaled, width, textureWidth, bytesPerPixel, height - 1);
				textureY++;
			}
