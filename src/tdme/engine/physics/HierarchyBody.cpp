...
void HierarchyBody::addBody(const string& id, const Transform& transform, const vector<BoundingVolume*> boundingVolumes) {
	//
	removeBody(id);
	//
	subBodies[id] = {
	...
	//
	Body::resetColliders(subBody->colliders, subBody->boundingVolumes, transform);
}

void HierarchyBody::updateBody(const string& id, const Transform& transform) {
	auto subBody = getBody(id);
	if (subBody == nullptr) return;
	//
	subBody->transform = transform;
	Body::resetColliders(subBody->colliders, subBody->boundingVolumes, transform);
}
