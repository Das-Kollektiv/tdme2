...
AnimationSetup::AnimationSetup(Model* model, const string& id, int32_t startFrame, int32_t endFrame, bool loop, const string& overlayFromNodeId, float speed)
{
	this->model = model;
	this->id = id;
...
int64_t AnimationSetup::computeDuration(int32_t startFrame, int32_t endFrame)
{
	return static_cast<int64_t>(
		static_cast<float>(endFrame - startFrame + 1) / model->getFPS() * 1000.0f * speed
	);
}
