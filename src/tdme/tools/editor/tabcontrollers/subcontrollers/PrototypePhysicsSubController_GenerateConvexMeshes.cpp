...
void PrototypePhysicsSubController_GenerateConvexMeshes::removeConvexMeshes(PrototypePhysicsSubController* prototypePhysicsSubController, Prototype* entityFinal)
{
	string meshPathName = prototypePhysicsSubController->view->getPopUps()->getFileDialogScreenController()->getPathName();
	string meshFileName = prototypePhysicsSubController->view->getPopUps()->getFileDialogScreenController()->getFileName();
...
void PrototypePhysicsSubController_GenerateConvexMeshes::generateConvexMeshes(PrototypePhysicsSubController* prototypePhysicsSubController, Prototype* entityFinal)
{
	...
	*/
	...
	if (convexMeshMode == "vhacd") {
		...
		//
		prototypePhysicsSubController->getView()->getPopUps()->getProgressBarScreenController()->show();
		...
		try {
			...
			vhacdParams.m_convexhullDownsampling = Tools::convertToInt(prototypePhysicsSubController->convexMeshesConvexHullDownSampling->getController()->getValue().getString());
			vhacdParams.m_alpha = Tools::convertToFloat(prototypePhysicsSubController->convexMeshesAlpha->getController()->getValue().getString());
			vhacdParams.m_beta = Tools::convertToFloat(prototypePhysicsSubController->convexMeshesBeta->getController()->getValue().getString());
			...
			*/
			...
			VHACDCallback vhacdCallback(prototypePhysicsSubController->getView()->getPopUps()->getProgressBarScreenController());
			...
			if (vhacdResult == true) {
				...
				for (auto i = 0; i < convexHulls; i++) {
					...
					auto convexHullFileName = meshFileName + ".cm." + to_string(i) + ".tm";
					Console::println(
						"PrototypePhysicsSubController_GenerateConvexMeshes::generateConvexMeshes(): VHACD: Saving convex hull@" +
						to_string(i) +
						": " + convexHullFileName +
			...
			prototypePhysicsSubController->view->getPopUps()->getInfoDialogScreenController()->show(
		...
		vhacd->Clean();
		vhacd->Release();
		prototypePhysicsSubController->getView()->getPopUps()->getProgressBarScreenController()->close();
	} else
		...
		try {
			...
			);
			{
				...
				for (auto i = 0; i < meshObject3DModel.getNodeCount(); i++) {
					...
					auto convexHullFileName = meshFileName + ".cm." + to_string(i) + ".tm";
					Console::println(
						"PrototypePhysicsSubController_GenerateConvexMeshes::generateConvexMeshes(): Model: Saving convex hull@" +
						to_string(i) +
						": " + convexHullFileName +
...
Model* PrototypePhysicsSubController_GenerateConvexMeshes::createModel(const string& id, vector<Triangle>& triangles) {
	auto model = new Model(id, id, UpVector::Y_UP, RotationOrder::XYZ, nullptr);
	auto material = new Material("primitive");
	material->setSpecularMaterialProperties(new SpecularMaterialProperties());
	material->getSpecularMaterialProperties()->setAmbientColor(Color4(0.5f, 0.5f, 0.5f, 1.0f));
