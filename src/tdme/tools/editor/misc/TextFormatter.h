...
private:
	...
	Language tscript = {
		...
		.keywordDelimiters = " \t\n;=-+*/%&|!~<>()[],",
		.keywordQuotes = "\"",
		.keywords1 = "getVariable setVariable unsetVariable add and array array.get array.set array.indexOf array.length array.push array.remove array.removeOf array.reverse array.sort bitwiseAnd bitwiseNot bitwiseOr bitwiseXor bool console.log div else elseif end equals float forCondition forTime greater greaterEquals if int json.deserialize json.serialize lesser lesserEquals map map.has map.get map.set map.getKeys map.getValues map.remove mat3.identity mat3.multiply mat3.rotate mat3.rotateAroundPoint mat3.rotateAroundTextureCenter mat3.scale mat3.translate mat4.computeEulerAngles mat4.identity mat4.invert mat4.multiply mat4.rotate mat4.scale mat4.translate math.DEG2RAD math.EPSILON math.G math.PI math.abs math.absmod math.acos math.asin math.atan math.atan2 math.ceil math.clamp math.cos math.exp math.floor math.log math.max math.min math.mod math.pow math.random math.round math.sign math.sin math.sqrt math.square math.tan mod mul not notEqual or prefixDecrement prefixIncrement quaternion.computeEulerAngles quaternion.computeMatrix quaternion.identity quaternion.invert quaternion.multiply quaternion.normalize quaternion.rotate return script.getNamedConditions script.getVariables script.call script.disableNamedCondition script.emit script.enableNamedCondition script.evaluate script.stop script.wait script.waitForCondition set set.has set.getKeys set.insert set.remove string string.isEmpty string.isFloat string.isInteger string.charAt string.concatenate string.endsWith string.equalsIgnoreCase string.firstIndexOf string.indexOf string.lastIndexOf string.length string.padLeft string.padRight string.regexMatch string.regexReplace string.replace string.space string.startsWith string.substring string.toLowerCase string.toUpperCase string.tokenize string.trim sub time.getAsString time.getCurrentMillis transform transform.AXIS_X transform.AXIS_Y transform.AXIS_Z transform.getRotationAngle transform.setRotationAngle transform.getRotationAxis transform.getRotationsQuaternion transform.getScale transform.setScale transform.getTransformMatrix transform.getTranslation transform.setTranslation transform.applyRotation transform.fromMatrix transform.interpolateRotation transform.multiply transform.rotate vec2 vec2.computeDotProduct vec2.computeLength vec2.computeLengthSquared vec2.getX vec2.getY vec2.normalize vec3 vec3.computeAngle vec3.computeCrossProduct vec3.computeDotProduct vec3.computeLength vec3.computeLengthSquared vec3.getX vec3.getY vec3.getZ vec3.normalize vec4 vec4.computeDotProduct vec4.computeLength vec4.computeLengthSquared vec4.getW vec4.getX vec4.getY vec4.getZ vec4.normalize xml.createTag",
