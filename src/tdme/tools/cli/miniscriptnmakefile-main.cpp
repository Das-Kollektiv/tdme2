...
{
	Console::printLine(string("miniscriptnmakefile ") + Version::getVersion());
	Console::printLine(Version::getCopyright());
	Console::printLine();

	//
	if (argc != 4) {
		Console::printLine("Usage: miniscriptnmakefile base_pathname source_pathname makefile_filename");
		return EXIT_FAILURE;
	}
