...
static void processFile(const string& scriptFileName, const string& miniscriptTranspilationFileName, const vector<string>& miniScriptExtensionFileNames) {
	...
	//
	auto miniScript = unique_ptr<TDMEMiniScript>(TDMEMiniScript::loadScript(Tools::getPathName(scriptFileName), Tools::getFileName(scriptFileName)));
		...
		for (auto i = 0; i < transpilationUnitCode.size(); i++) {
			...
			if (StringTools::startsWith(trimmedLine, "registerMethod") == true ||
				...
				if (StringTools::firstIndexOf(StringTools::substring(trimmedLine, 14), "new") == string::npos) {
					Console::println("src/tdme/utilities/TDMEMiniScript.cpp: registerMethod @ " + to_string(i) + ": '" + trimmedLine + "': unable to determine class name");
				} else {
	...
	string generatedExecuteCode;
	{
		...
		for (const auto& script: scripts) {
			// method name
			string methodName =
				(script.scriptType == TDMEMiniScript::Script::SCRIPTTYPE_FUNCTION?
					"":
					(script.scriptType == TDMEMiniScript::Script::SCRIPTTYPE_ON?"on_":"on_enabled_")
				) +
	...
	generatedDeclarations+= headerIndent + "\t" + "\t" + "TDMEMiniScript::startScript();" + "\n";
	...
	generatedDeclarations+= headerIndent + "\t" + "if (native == false) {" + "\n";
	generatedDeclarations+= headerIndent + "\t" + "\t" + "TDMEMiniScript::execute();" + "\n";
	generatedDeclarations+= headerIndent + "\t" + "\t" + "return;" + "\n";
	generatedDeclarations+= headerIndent + "\t" + "}" + "\n";
	generatedDeclarations+= headerIndent + "\t" + "auto& scriptState = getScriptState();" + "\n";
	...
	registerMethodsDefinitions+= methodCodeIndent+ "TDMEMiniScript::registerMethods();" + "\n";
	...
	emitDefinition+= methodCodeIndent + "\t" + "TDMEMiniScript::emit(condition);" + "\n";
	...
	initializeNativeDefinition+= methodCodeIndent + "\t" + "{" + "\n";
	{
		...
		for (const auto& script: scripts) {
			...
			initializeNativeDefinition+= methodCodeIndent + "\t" + "\t" + "\t" + (script.scriptType == TDMEMiniScript::Script::SCRIPTTYPE_FUNCTION?"Script::SCRIPTTYPE_FUNCTION":(script.scriptType == TDMEMiniScript::Script::SCRIPTTYPE_ON?"Script::SCRIPTTYPE_ON":"Script::SCRIPTTYPE_ONENABLED")) + "," + "\n";
			...
			initializeNativeDefinition+= methodCodeIndent + "\t" + "\t" + "\t" + "{" + "\n";
			auto statementIdx = TDMEMiniScript::STATEMENTIDX_FIRST;
	...
	generatedDetermineScriptIdxToStartDefinition+= string() + "\t" + "if (native == false) return TDMEMiniScript::determineScriptIdxToStart();" + "\n";
	...
	generatedDetermineNamedScriptIdxToStartDefinition+= string() + "\t" + "if (native == false) return TDMEMiniScript::determineNamedScriptIdxToStart();" + "\n";
	...
	generatedDetermineNamedScriptIdxToStartDefinition+= string() + "\t" + "for (const auto& enabledNamedCondition: enabledNamedConditions) {" + "\n";
	{
		...
		for (const auto& script: scripts) {
			...
			// emit code
			if (script.scriptType == TDMEMiniScript::Script::SCRIPTTYPE_ON && StringTools::regexMatch(script.condition, "[a-zA-Z0-9_]+") == true) {
			...
			// declaration
			generatedDeclarations+= headerIndent + "/**" + "\n";
			generatedDeclarations+= headerIndent + " * Miniscript transpilation of: " + (script.scriptType == TDMEMiniScript::Script::SCRIPTTYPE_FUNCTION?"function":(script.scriptType == TDMEMiniScript::Script::SCRIPTTYPE_ON?"on":"on-enabled")) + ": " + script.condition + (script.name.empty() == false?" (" + script.name + ")":"") + "\n";
			generatedDeclarations+= headerIndent + " * @param miniScriptGotoStatementIdx TDMEMiniScript goto statement index" + "\n";
			generatedDeclarations+= headerIndent + " */" + "\n";
			...
			if (script.emitCondition == false) {
				if (script.scriptType == TDMEMiniScript::Script::SCRIPTTYPE_ONENABLED) {
				...
				MiniScriptTranspiler::transpileScriptCondition(
					miniScript.get(),
					script.scriptType == TDMEMiniScript::Script::SCRIPTTYPE_ON?generatedDetermineScriptIdxToStartDefinition:generatedDetermineNamedScriptIdxToStartDefinition,
					scriptIdx,
					methodCodeMap,
					allMethods,
					"-1",
					"bool returnValueBool; returnValue.getBooleanValue(returnValueBool); if (returnValueBool == true) return " + to_string(scriptIdx) + ";",
					script.scriptType == TDMEMiniScript::Script::SCRIPTTYPE_ONENABLED?1:0
				);
			}
