...
void SharedModelViewerView::updateGUIElements()
{
	if (entity != nullptr) {
		...
		modelViewerScreenController->setEntityProperties(preset != nullptr ? preset->getValue() : static_cast< String* >(nullptr), entity->getProperties(), nullptr);
...
void SharedModelViewerView::loadSettings()
{
	...
	Object* tmp;
	try {
		fis = new FileInputStream(u"./settings/modelviewer.properties"_j);
